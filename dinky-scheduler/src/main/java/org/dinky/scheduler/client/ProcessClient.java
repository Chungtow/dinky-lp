...
public class ProcessClient {
    ...
    /**
     * Release a process definition (set it ONLINE).
     *
     * @param projectCode project code
     * @param processCode process definition code
     * @return true if released successfully
     */
    public boolean releaseProcessDefinition(Long projectCode, Long processCode) {
        Map<String, Object> map = new HashMap<>();
        map.put("projectCode", projectCode);
        map.put("code", processCode);
        String format = StrUtil.format(
        ...
        try (HttpResponse httpResponse = HttpRequest.post(format)
                ...
                .execute()) {
            String content = httpResponse.body();
            MyJSONUtil.verifyResult(MyJSONUtil.toBean(content, new TypeReference<Result<Object>>() {}));
            return true;
        }
    }

    /**
     * Delete a process definition by code.
     *
     * @param projectCode project code
     * @param processCode process definition code to delete
    ...
    public boolean deleteProcessDefinition(Long projectCode, Long processCode) {
        ...
        try (HttpResponse httpResponse = HttpRequest.delete(format)
                .header(
                        Constants.TOKEN,
                        SystemConfiguration.getInstances()
                                .getDolphinschedulerToken()
    ...
    /**
     * Start (execute) a process instance.
     *
     * @param projectCode project code
     * @param processCode process definition code to start
     * @return true if started successfully
     */
    public boolean startProcessInstance(Long projectCode, Long processCode) {
        Map<String, Object> map = new HashMap<>();
        map.put("projectCode", projectCode);
        String format = StrUtil.format(
                SystemConfiguration.getInstances().getDolphinschedulerUrl().getValue()
